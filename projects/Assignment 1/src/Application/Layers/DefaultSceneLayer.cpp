...
void DefaultSceneLayer::_CreateScene()
{
	...
	if (loadScene && std::filesystem::exists("scene.json")) {
		...
		Texture2D::Sptr    breakTex = ResourceManager::CreateAsset<Texture2D>("textures/bricks_diffuse discard.png");
		...
		Texture3D::Sptr Cuslut = ResourceManager::CreateAsset<Texture3D>("luts/custom.cube");
		
		// Configure the color correction LUT
		scene->SetColorLUT(Clut);
		//scene->SetColorLUT(Wlut);
		//scene->SetColorLUT(Cuslut);
